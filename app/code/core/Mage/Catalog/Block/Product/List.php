...
class Mage_Catalog_Block_Product_List extends Mage_Catalog_Block_Product_Abstract {
    ...
    public function getLoadedProductCollection() {
        $layer = $this->getLayer();
        $sort_by = Mage::getStoreConfig('catalog/frontend/default_sort_by');
        $grid_per_page = Mage::getStoreConfig('catalog/frontend/grid_per_page');
        if (!(int) $grid_per_page):
            $grid_per_page = 8;
        endif;
        $toolbar = new Mage_Catalog_Block_Product_List_Toolbar();

        $order = $toolbar->_getData('_current_grid_order');
        
        if (!$order){
            $orders = $toolbar->getAvailableOrders();
            $defaultOrder = $toolbar->_orderField = $sort_by;
            ...
            if ($order && isset($orders[$order])) {
                if ($order == $defaultOrder) {
                    ...
                    $toolbar->_memorizeParam('sort_order', $order);
                }
            } else {
        ...
        $currentOrder = array($order=>$orders[$order]);
        unset($orders[$order]);
//        $orders = array_merge($currentOrder,$orders);
        $currentPage = (int) Mage::app()->getRequest()->getParam('p', 1);
//        echo '<pre>';
//        print_r( $this->getPage());
//        echo '</pre>';
//        exit;
        $limit = $currentPage * $grid_per_page;
        $currentPage = 1;
        if ($this->getRequest()->isXmlHttpRequest()):
            $currentPage = Mage::app()->getRequest()->getParam('p', 1);
            $limit = $grid_per_page;
        endif;
        $origCategory = $layer->getCurrentCategory();

//         = $this->_getProductCollection()->addAttributeToSelect('*')->groupByAttribute(array('stylecode','color'));
        ...
        $collection
                ...
                ->setOrder($sort_by, 'desc')
//                ->setOrder('id', 'desc')
                ->setPage($currentPage, $limit)
                ->addCategoryFilter($origCategory);
//        echo '<pre>';
//        print_r(get_class_methods($collection));
//        echo '</pre>';
//        exit;
        foreach ($orders as $key => $value):
             $collection->addOrder($key, 'desc');
        endforeach;
//        echo '<pre>';
//        print_r($collection->getSelect()->__toString());
//        echo '</pre>';
//        exit;
        return $collection; //$this->_getProductCollection();
    }
