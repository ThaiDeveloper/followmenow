        ...
        foreach ($parentProducts as $parentProduct) {
            ...
            foreach ($configAttributes as $attribute) {
                ...
                foreach ($parentProduct->getChildrenProducts() as $childProduct) {
                    ...
                    if ($attribute->getAttributeId() == $listSwatchAttr->getAttributeId() && !in_array($mapping[$optionLabel]['label'], $listSwatchValues)
                    ) {
            ...
            foreach ($mapping as $map) {
                ...
                if ($imagePath) {
                    $imagesByLabel[$map['label']]['configurable_product']
                            [Mage_ConfigurableSwatches_Helper_Productimg::MEDIA_IMAGE_TYPE_SMALL] = $this->_resizeProductImage($product, 'small_image', $keepFrame, $imagePath);

                    $imagesByLabel[$map['label']]['configurable_product']
                            [Mage_ConfigurableSwatches_Helper_Productimg::MEDIA_IMAGE_TYPE_BASE] = $this->_resizeProductImage($product, 'image', $keepFrame, $imagePath);
                }
            }
...
//                            $image = $this->_resizeProductImage($childProduct, $imageType, $keepFrame);
                            $image = Mage::getModel('catalog/product_media_config')->getMediaUrl($childProduct->getSmallImage());
                        }
                        if ($imageType == 'image') {
//                            $image = $this->_resizeProductImage($childProduct, $imageType, $keepFrame);
                            $checkImage = Mage::getModel('catalog/product_media_config')->getMediaUrl($childProduct->getImage());
                            if(strpos($checkImage, 'jpg')){
                                $image = $checkImage;
                            }else{
                               $image = $this->_resizeProductImage($childProduct, $imageType, $keepFrame);
                            }    
...
//                }
                ...
                if ($this->_resizeProductImage($product, $imageType, $keepFrame, null, true)) {
                    if ($imageType == 'small_image') {
//                            $image = $this->_resizeProductImage($childProduct, $imageType, $keepFrame);
