...
class JR_AttributeOptionImage_Block_Catalog_Product_Attribute_Edit_Tab_Options extends Mage_Adminhtml_Block_Catalog_Product_Attribute_Edit_Tab_Options
{
    ...
    public function getTeamList(){
        $teamCollection = Mage::getModel('ourteam/ourteam')->getCollection();
        $select = $teamCollection->removeAllFieldsFromSelect()->addFieldToSelect('name');
        $teamData = array();
        foreach ($select as $item) {
            $teamData[$item['entity_id']] = $item['name'];
        }
        return $teamData;
    }
}