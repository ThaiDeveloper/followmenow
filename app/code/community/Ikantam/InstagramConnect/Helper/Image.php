...
class Ikantam_InstagramConnect_Helper_Image extends Mage_Core_Helper_Abstract {
    ...
    public function runUpdate($url, $tag) {
        ...
        if (isset($result['nextUrl'], $result['nextMaxId'])) {
            ...
            $maxPhotoCount = (int) Mage::getStoreConfig('ikantam_instagramconnect/module_options/max_photo_count');
            if ($maxPhotoCount <= 0) {
                $maxPhotoCount = self::MAX_PHOTO_COUNT;
            }
            while ($endpointUrl && ($maxTagId > $savedId) && ($imagesCount < $maxPhotoCount)) {
    ...
    public function getHomeTags($bytag = true) {
        $rawTags = Mage::getStoreConfig('ikantam_instagramconnect/module_options/homeusers');
        $tags = explode(',', $rawTags);
