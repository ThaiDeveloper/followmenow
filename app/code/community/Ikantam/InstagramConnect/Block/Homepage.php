...
//        $images = new Varien_Data_Collection();
        ...
        $tags = Mage::helper('instagramconnect/image')->getHomeTags();
        $usernames = Mage::helper('instagramconnect/image')->getHomeTags(false);
        $limit = (int)Mage::helper('instagramconnect')->getHomePageLimit();
        if (count($tags)) {
            ...
            $select = $tagsCollection->getSelect()
                    ...
                    ->where('tag IN (\''.implode("','", $tags).'\') OR username IN (\''.implode("','", $usernames).'\')');
            if($limit){
                 $select->limit($limit);
            }
