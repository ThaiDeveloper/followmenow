...
var ProductMediaManager = {
    ...
    moreImage: function () {
        field = jQuery('div.more-images');
        clone = jQuery('div.more-images').clone();
        field.addClass('hidden');
    },
    /*
     * Auto choose color if have 1 color
     */
    chooseProduct: function () {
        var numImg = jQuery('ul#configurable_swatch_color img');
        var count = numImg.length;
        if (count == 1) {
            setTimeout(function () {
                jQuery('ul#configurable_swatch_color li:first-child img').trigger('click');
            }, 500);
        }
        if(jQuery('a.currentColorSwatch').length){
           jQuery('a.currentColorSwatch')[0].click();
            
        }
                
    },
    loadStoreMap: function () {
        var dealermap = jQuery('#store-list-map-canvas');
        if (dealermap.length) {
                    ...
                    if ($datas.length > 2) {
                        if ($datas.length > 0) {
                            ...
                            jQuery.each($datas, function ($key, $value) {
                                ...
                                $html += '<div class="stores-view">';
                                $html += '<p class="store-name"><a href="' + $urlDealer + '" target="_blank">'+ $value.name + '</a></p>'
                                
                                
//                                if(typeof($value.url)!=='undefined' && $value.url){
//                                    $html += '<p>'+ $value.url+'</p>'; 
//                                }
                                if(typeof($value.street_address)!=='undefined'){
                                    $html += '<p>'+ $value.street_address; 
                                }
                                ...
                                if(typeof($value.state)!=='undefined'){
                                    $html += '<p>'+ $value.state + '</p>';
                                }
                                if(typeof($value.countryname)!=='undefined'){
                                    $html += '<p>'+ $value.countryname + '</p>';
                                }
                                if(typeof($value.phone)!=='undefined' && $value.phone){
                                    $html += '<p>'+ $value.phone+'</p>'; 
                                }
                                if(typeof($value.distance)!=='undefined'){
                                    $html += '<p>'+ Math.round($value.distance * 100) / 100  + ' Km</p>';
                                }
                                 $html += '</div>';
                            });
...
//                            console.log(markers);
                            ...
                            for (var i = 0; i < markers.length; i++) {
                                ProductMediaManager.addMarker(markers[i], map, infowindow);
                                var data = markers[i];
    ...
    colorBox: function () {
        if (jQuery(".showImage").length > 0)
        {
            jQuery(".showImage").colorbox({rel: 'showImage', transition: "fade"});
        }
    }
};

$j(document).ready(function () {
    ProductMediaManager.colorBox();
    ProductMediaManager.loadStoreMap();
    ProductMediaManager.init();
    ProductMediaManager.moreImage();
    ProductMediaManager.chooseProduct();
});
